...
export default function ConfigForm() {
  ...
  const [bindingModes, setBindingModes] =
    useState<
  ...
  if (node) {
    for (const categoryRegistry of Object.values(
      nodeRegistry
    )) {
      if (
        node.definitionType in categoryRegistry
      ) {
  ...
  const metadata =
    foundNodeDef.ui.metadata;

  const fields = foundNodeDef.ui.fields;

  const CategoryIcon =
    foundCategory?.metadata.icon;
      ...
      >
        ...
        <div className="p-6 flex flex-col gap-5 max-h-100 overflow-y-auto">
          ...
          fields.map((field: any) => {
            const mode =
              bindingModes[field.key] ||
              "static";

            const currentValue =
              node.config[field.key] as any;
            ...
            return (
              ...
              >
                ...
                {mode === "binding" && (
                  <select
                    value={
                      currentValue?.sourceField ||
                      ""
                    }
                    onChange={(e) => {
                      const selected =
                        compatibleBindings.find(
                          (
                            b: any
                          ) =>
                            b.field ===
